<script lang="tsx">
  ...
  import { CreateComponentGroupType, CreateComponentType, PublicConfigType } from '/@/packages/types';
  ...
  export default defineComponent({
    setup() {
      ...
      const minDistance = computed(() => {
        return settingStore.state.chartAlignRange;
      });
      ...
      // * 当前目标
      const selectId = computed(() => editStore.state.targetChart.selectId);
      const selectTarget = computed(() => editStore.state.componentList[editStore.fetchTargetIndex()]);
      const selectAttr = computed((): Partial<PublicConfigType['attr']> => selectTarget.value?.attr || {});
      ...
      const canvasPositionList = computed(() => {
        return {
          id: '0',
          attr: {
            w: cloneDeep(editStore.state.editCanvasConfig.width),
            h: cloneDeep(editStore.state.editCanvasConfig.height),
            x: 0,
            y: 0,
            offsetX: 0,
            offsetY: 0,
      ...
      watch(
        ...
        throttle(() => {
          try {
            ...
            // 循环查询所有组件数据
            const componentList = editStore.state.componentList.map((e: CreateComponentType | CreateComponentGroupType) => {
              return {
                id: e.id,
                attr: e.attr,
              };
      ...
      return () => (
        <div class="edit-align-line">
          {line.lineArr.map((i) => (
            <div class={['line', i.includes('row') ? 'row' : 'col', line['select'].has(i) && 'visible']} style={useComponentStyle(line['select'].get(i))}></div>
          ))}
        </div>
      );
    },
